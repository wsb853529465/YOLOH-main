...
__all__ = ['ResNet', 'resnet18', 'resnet34', 'resnet50', 'resnet101',
           ...
           'wide_resnet50_2', 'wide_resnet101_2', 'cspdarknet53']
...
class DarkNet(nn.Module):
    ...
    def __init__(
        ...
        for i, num_blocks in enumerate(self.stage_blocks):
            ...
            layer_name = 'layer{}'.format(i + 1)  # i == res (i)
...
def _darknet(
    ...
    **kwargs: Any
    ...
    model = DarkNet(depth=53, with_csp=True, res5_dilation=2, **kwargs)
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls[arch],
                                              progress=progress)
        model.load_state_dict(state_dict, strict=False)
    else:
        net = torch.load('C:/sbj/YOLOF_CSP_D_53_DC5_3x.pth')
        # print(net)
...
def cspdarknet53(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    r"""darknet53 model from
    `"Wide Residual Networks" <https://arxiv.org/pdf/1605.07146.pdf>`_.

    The model is the same as ResNet except for the bottleneck number of channels
    which is twice larger in every block. The number of channels in outer 1x1
    convolutions is the same, e.g. last block in ResNet-50 has 2048-512-2048
    channels, and in darknet53 has 1024-1024.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _darknet('cspdarknet53', pretrained, progress, **kwargs)  # DarkBlock, [1, 2, 8, 8, 4],
